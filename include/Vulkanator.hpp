...
namespace Vulkanator
{
	...
	struct GlobalParams
	{
		...
		// Due to the definition of render pass compatibility, we have to make it three times
		// for each render format depth
		...
		// the pipeline's layout and Descriptor set layout which basically will describe all the inputs
		// that each of the shader stages takes in as input such as uniform-buffers,
		// sampled-images, storage-images, and so on
	...
	{
		// Each instance of the effect will get a command buffer it may use to generate GPU workloads
		vk::UniqueCommandBuffer CommandBuffer = {};
		// Each instance of the effect will get a synchronization fence it may use when submitting GPU workloads
		vk::UniqueFence Fence = {};

		// Each instance of the effect will get a descriptor set to pass it's uniform data over to the shader
		vk::UniqueDescriptorSet DescriptorSet = {};

		// The actual buffer that will hold the uniform buffer that the descriptor set will point to
		...
		struct SequenceCache
		{
			...
			// Then we should resize the entire buffer to be smaller if it is this percentage
			// smaller than the cached size

			// If an allocation comes in that is %15 smaller than the cache, then the buffer to fit
			...
			// We use these structs so that we can easily "==" compare the image in the cache
			// with any new requests coming in
	...
	// In vulkan clip space, {0,0} is on the top left of the image, with Y increasing downwards
