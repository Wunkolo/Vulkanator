...
namespace VulkanUtils
{
	...
	// 0: eR8G8B8A8Unorm,
	//
	// While this is the correct datatype, remember that After Effects uses PF_MAX_CHAN16 rather than 0xFFFF as "white"
	// so the maximum color value is actually `32768`(0x8000), not `65535`(0xFFFF). So be sure to update your shaders accordingly
	// 1: eR16G16B16A16Unorm,
	//
	...
	std::optional<vk::UniqueDeviceMemory> AllocateDeviceMemory(
		vk::Device Device,
		std::size_t Size, std::uint32_t MemoryTypeIndex,
	...
	> AllocateBuffer(
		...
		vk::MemoryPropertyFlags Properties, vk::MemoryPropertyFlags ExcludeProperties = vk::MemoryPropertyFlagBits::eProtected,
