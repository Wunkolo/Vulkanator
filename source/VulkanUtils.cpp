...
namespace VulkanUtils
{
	...
	)
	{
		...
		if (auto AllocResult = Device.allocateMemoryUnique(AllocInfo); AllocResult.result == vk::Result::eSuccess)
		{
	...
	> AllocateBuffer(
		...
		std::size_t Size, vk::BufferUsageFlags Usage,
		vk::MemoryPropertyFlags Properties, vk::MemoryPropertyFlags ExcludeProperties,
	...
	)
	{
		...
		if (auto BufferResult = Device.createBufferUnique(NewBufferInfo); BufferResult.result == vk::Result::eSuccess)
		{
		...
		const vk::MemoryRequirements NewBufferRequirements = Device.getBufferMemoryRequirements(NewBuffer.get());

		const auto BufferMemoryIndex = FindMemoryTypeIndex(
			PhysicalDevice, NewBufferRequirements.memoryTypeBits,
			Properties, ExcludeProperties
		);

		if (BufferMemoryIndex < 0) return std::nullopt;

		vk::UniqueDeviceMemory NewBufferDeviceMemory{};
		if (
			auto NewDeviceMemory = AllocateDeviceMemory(
		...
		else return std::nullopt; // Error allocating device memory

		try
		{
			Device.bindBufferMemory(NewBuffer.get(), NewBufferDeviceMemory.get(), 0);
		}
		...
		return std::make_tuple(std::move(NewBuffer), std::move(NewBufferDeviceMemory));
	}
	...
	> AllocateImage(
		vk::Device Device, vk::PhysicalDevice PhysicalDevice,
		vk::ImageCreateInfo NewImageInfo,
		vk::MemoryPropertyFlags Properties, vk::MemoryPropertyFlags ExcludeProperties
	)
	{
		vk::UniqueImage NewImage = {};

		if (auto ImageResult = Device.createImageUnique(NewImageInfo); ImageResult.result == vk::Result::eSuccess)
		{
		...
		const vk::MemoryRequirements NewImageRequirements = Device.getImageMemoryRequirements(NewImage.get());

		const auto ImageMemoryIndex = FindMemoryTypeIndex(
			PhysicalDevice, NewImageRequirements.memoryTypeBits,
			Properties, ExcludeProperties
		);

		if (ImageMemoryIndex < 0) return std::nullopt; // Unable to find suitable memory index for buffer

		vk::UniqueDeviceMemory NewImageDeviceMemory{};
		if (
			auto NewDeviceMemory = AllocateDeviceMemory(
				Device, NewImageRequirements.size,
				ImageMemoryIndex,
				vk::Buffer(), NewImage.get() // ShouldDedicate ? NewImage.get() : vk::Image()
			)
			)
		{
		...
		return std::make_tuple(std::move(NewImage), std::move(NewImageDeviceMemory));
	}

	std::optional<vk::UniqueShaderModule> LoadShaderModule(
	...
	)
	{
		...
		ShaderModuleInfo.pCode = reinterpret_cast<const std::uint32_t*>(Code.data());

		vk::UniqueShaderModule ShaderModule = {};
		if (auto ShaderModuleResult = Device.createShaderModuleUnique(ShaderModuleInfo); ShaderModuleResult.result == vk::Result::eSuccess)
		{
	...
	// This function will be called whenever the Vulkan backend has something to say about what you are doing
	// How ever you want to handle this, implement it here.
